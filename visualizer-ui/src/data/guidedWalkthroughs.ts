  ...
  ...actionCards([
    {
      ...
      animationBreakdown: { count: 3, items: ['Stack variable "head": the declaration creates this new local reference.', "Memory [Object 1]: the new keyword allocates a separate Node object.", 'Field "value = 10": the Node constructor stores 10 inside that object.'] },
    },
    {
      ...
      animationBreakdown: { count: 3, items: ['Stack variable "temp": the declaration creates a second local reference.', "Memory [Object 2]: new allocates a different Node instead of changing [Object 1].", 'Field "value = 20": the constructor stores 20 inside the new Node.'] },
    },
    {
      ...
      animationBreakdown: { count: 3, items: ['"temp": Java reads the [Object 2] reference that will be copied.', '"head": Java follows this reference to find [Object 1], the object being changed.', '"[Object 1].next": Java stores the copied [Object 2] reference here, creating the link.'] },
    },
    {
      ...
      animationBreakdown: { count: 3, items: ['Stack variable "value": Java creates the destination that will hold the copied 10.', '"head": Java follows this reference to locate [Object 1].', '"[Object 1].value": Java reads 10 from this source field; the original value stays there.'] },
    },
  ]),
];
...
const stackSteps: WalkthroughStepDef[] = [
  ...
  ...actionCards([
    { lessonStep: 1, line: 3, code: "MyStack s = new MyStack();", runTitle: "Create the stack tracker", runExplanation: 'new MyStack() creates one MyStack object. Its top field starts as null because no Node has been pushed yet; "s" will point to it.', observeTitle: "An empty stack was created", observeExplanation: 'The Stack variable "s" points to [Object 1], the MyStack object in memory. [Object 1].top is null, which means the stack contains no Nodes.', animationBreakdown: { count: 3, items: ['Stack variable "s": the declaration creates the local reference.', 'Memory [Object 1]: new MyStack() allocates the separate stack-tracker object.', 'Field "top = null": a new MyStack starts empty, so its top reference has no Node yet.'] } },
    { lessonStep: 2, line: 4, code: "Node n1 = new Node(42);", runTitle: "Create the first node", runExplanation: 'new Node(42) creates a Node whose value is 42 and next is null. "n1" will point to it, but it is not the stack top yet.', observeTitle: "The first node is ready", observeExplanation: '"n1" points to [Object 2], a Node containing 42. [Object 1].top is still null, so the Node has been created but not pushed.', animationBreakdown: { count: 3, items: ['Stack variable "n1": Java creates a reference for the first Node.', 'Memory [Object 2]: new allocates a Node separate from the MyStack object.', 'Field "value = 42": the Node constructor stores 42 inside [Object 2].'] } },
    { lessonStep: 3, line: 5, code: "s.top = n1;", runTitle: "Push the first node", runExplanation: 'Java follows "s" to [Object 1] and copies the reference in "n1" into its top field.', observeTitle: "The stack now contains 42", observeExplanation: 'The moving [Object 2] label is a copied reference. [Object 1].top now points to the Node containing 42, making it both the top and bottom Node.', animationBreakdown: { count: 3, items: ['"s": Java reads this reference to locate the MyStack object.', '"n1": Java reads the [Object 2] reference that will become the top.', '"[Object 1].top": Java stores the copied [Object 2] reference here.'] } },
    { lessonStep: 4, line: 7, code: "Node n2 = new Node(84);", runTitle: "Create another node", runExplanation: 'This creates a separate Node containing 84. "n2" points to it, but the existing stack still starts at the Node containing 42.', observeTitle: "The second node is ready", observeExplanation: '"n2" points to [Object 3], while [Object 1].top still points to [Object 2]. The new Node is not connected to the stack yet.', animationBreakdown: { count: 3, items: ['Stack variable "n2": Java creates another Node reference.', 'Memory [Object 3]: new allocates a second, separate Node.', 'Field "value = 84": the constructor stores 84 inside [Object 3].'] } },
    { lessonStep: 5, line: 8, code: "n2.next = s.top;", runTitle: "Link the new node to the old top", runExplanation: 'Java copies the current top reference from [Object 1].top into [Object 3].next. This preserves the Node already on the stack.', observeTitle: "The old stack is preserved", observeExplanation: 'The moving [Object 2] is a reference copied into [Object 3].next. The new Node containing 84 now points down to the old Node containing 42.', animationBreakdown: { count: 3, items: ['"s": Java follows this reference to read the current top.', '"n2": Java follows this reference to find the new Node being changed.', '"[Object 3].next": Java stores the copied [Object 2] reference here, preserving the old stack.'] } },
    { lessonStep: 6, line: 9, code: "s.top = n2;", runTitle: "Make 84 the new top", runExplanation: 'Java copies the reference in "n2" into [Object 1].top. The tracker will now start at the new Node.', observeTitle: "Push complete", observeExplanation: 'The stack order is now top -> 84 -> 42. [Object 1].top points to [Object 3], and [Object 3].next keeps the link to [Object 2].', animationBreakdown: { count: 3, items: ['"s": Java follows this reference to the stack tracker.', '"n2": Java reads the [Object 3] reference that will become the new top.', '"[Object 1].top": Java replaces the old top reference with [Object 3].'] } },
  ]),
];
...
const liveTraceSteps: WalkthroughStepDef[] = [
  ...
  ...actionCards([
    { lessonStep: 1, line: 3, code: "int x = 5;", runTitle: "Create x", runExplanation: 'This declares an integer variable named "x" in the main stack frame and stores 5 directly in it.', observeTitle: "x now holds 5", observeExplanation: 'The main stack frame now shows x = 5. Integers are primitive values, so x stores the number itself rather than a reference to an object.', animationBreakdown: { count: 1, items: ['Stack variable "x": Java creates this local destination and stores the primitive value 5 directly in it.'] } },
    { lessonStep: 2, line: 4, code: "int y = 10;", runTitle: "Create y", runExplanation: 'This declares another integer variable named "y" in the same main stack frame and stores 10 in it.', observeTitle: "y now holds 10", observeExplanation: "The main frame now contains x = 5 and y = 10. Each variable has its own value.", animationBreakdown: { count: 1, items: ['Stack variable "y": the declaration creates a second local destination and stores 10 in it.'] } },
    { lessonStep: 3, line: 5, code: "int result = multiply(x, y);", runTitle: "Call multiply", runExplanation: "Java reads x and y, copies 5 and 10 into the method parameters a and b, and pushes a new multiply frame above main.", observeTitle: "A method frame was pushed", observeExplanation: "The multiply frame is now on top with a = 5 and b = 10. The main frame remains underneath, paused until multiply returns.", animationBreakdown: { count: 2, items: ['Parameter "a": Java copies the value 5 from x into the new multiply frame.', 'Parameter "b": Java copies the value 10 from y into that same frame.'] } },
    { lessonStep: 4, line: 10, code: "return a * b;", runTitle: "Evaluate the multiplication", runExplanation: "Inside multiply, Java reads a as 5 and b as 10, then evaluates the expression 5 * 10.", observeTitle: "The expression produces 50", observeExplanation: "The multiply frame remains visible while Java computes the expression. The next trace event will return the resulting 50 to main.", animationBreakdown: { count: 1, items: ['Expression result: Java evaluates a * b and places the computed 50 in the calculation row.'] } },
    { lessonStep: 5, line: 10, code: "return a * b;", runTitle: "Return 50 to main", runExplanation: "The return keyword sends the computed integer 50 back to the waiting call in main. multiply is now finished.", observeTitle: "multiply returns 50", observeExplanation: "A temporary return value of 50 is shown in the multiply frame. That frame is ready to be removed; a and b will disappear with it.", animationBreakdown: { count: 1, items: ['Temporary "return value": Java records 50 in the finishing multiply frame before sending it back to main.'] } },
    { lessonStep: 6, line: 5, code: "int result = multiply(x, y);", runTitle: "Store the returned value", runExplanation: 'Execution resumes in main. The returned 50 is assigned to a new integer variable named "result".', observeTitle: "result now holds 50", observeExplanation: 'The multiply frame is gone. The main frame now contains x = 5, y = 10, and result = 50.', animationBreakdown: { count: 1, items: ['Stack variable "result": after multiply finishes and its frame pops, the returned 50 is stored here in main.'] } },
    { lessonStep: 7, line: 6, code: "System.out.println(\"Result = \" + result);", runTitle: "Print the result", runExplanation: 'Java joins the text "Result = " with the value in result, producing "Result = 50", then sends that text to the program output.', observeTitle: "Program complete", observeExplanation: 'The output panel displays "Result = 50". main has reached the end of its visible instructions, so the traced program is finished.', animationBreakdown: { count: 1, items: ['Program output: System.out.println sends the completed text "Result = 50" to the stdout panel.'] } },
  ]),
];
